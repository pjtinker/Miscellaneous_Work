# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 20:20:02 2017
...
os.chdir('D:\\TTU\\NEL_Research\\SD Work\\CAD')


clf = RandomForestClassifier()
rus = RandomUnderSampler(random_state=1337)
data = pd.read_csv('cad-discretized.csv')


X = data.drop(['Cath', 'LAD', 'LCX', 'RCA', 'Exertional CP'], axis=1)
y = data.iloc[:, -1].ravel()


features = SelectKBest(f_classif, k=3).fit(X, y)

# Grab three important feature
mif = np.argmax(features.scores_)
#mif = np.argpartition(features.scores_, -3)[-3:]
f = X.columns[mif]
