# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 20:07:19 2017
...
ro.r('''
     ...
     ''')
ck = ro.globalenv['ck']
os.chdir('D:\\TTU\\NEL_Research\\SD Work\\PimaIndians\\SDToCkmeans')
...
data_not_true = data[data.Class == 0]
columns = ['SG', 'Rule', 'SG_size', 'count-cluster-1', 'count-cluster-2', 'Accuracy', 'AUROC', 'len_hf1', 'len_hf2', 'len_nothf', 'mean_hf1', 'mean_hf2', 'mean_nothf','hf1_tvalue_hf2', 'hf1_pvalue_hf2', 'hf1_tvalue_nothf', 'hf1_pvalue_nothf', 'hf1_tvalue_oneSamp','hf1_pvalue_oneSamp']
...
classifier = learner(Orange.data.Table('pima-indians-sdtock-orange-format.csv'))
X['Cluster_feature'] = 0
...
for i in range(0,3):
    # Set sg equal to the samples that are covered by the rule
     ...
     # Grab len(hf1) random samples where Class == False to compare accuracy/auroc to SD groups
     ...
     results = results.append({'SG':'SG' + str(i) +'_Class_0', 'Rule' : str(classifier.rule_list[i]), 'SG_size': sg.shape[0], 'count-cluster-1': len(sg[sg.Cluster2 == 1]), 'count-cluster-2': len(sg[sg.Cluster2 == 2]), 'Accuracy':accuracy, 'AUROC':roc_auc_score(y, current_probabilities), 'len_hf1':len(hf1), 'len_hf2':len(hf2), 'len_nothf': len(not_hf), 'mean_hf1':np.mean(hf1)[0], 'mean_hf2':np.mean(hf2)[0], 'mean_nothf':np.mean(not_hf)[0], 'hf1_tvalue_hf2': tvalue_sg[0],'hf1_pvalue_hf2': pvalue_sg[0], 'hf1_tvalue_nothf': pvalue_all[0], 'hf1_pvalue_nothf':pvalue_all[0], 'hf1_tvalue_oneSamp':tvalue_one[0],'hf1_pvalue_oneSamp':pvalue_one[0]}, ignore_index = True)
     X['Cluster_feature'] = 0
...
for i in range(0,3):
     ...
     # Write it all to the results dataframe.  
     results = results.append({'SG':'SG' + str(i) +'_Class_1', 'Rule' : str(classifier.rule_list[count + i]), 'SG_size': sg.shape[0], 'count-cluster-1': len(sg[sg.Cluster2 == 1]), 'count-cluster-2': len(sg[sg.Cluster2 == 2]), 'Accuracy':accuracy, 'AUROC':roc_auc_score(y, current_probabilities), 'len_hf1':len(hf1), 'len_hf2':len(hf2), 'len_nothf': len(not_hf), 'mean_hf1':np.mean(hf1)[0], 'mean_hf2':np.mean(hf2)[0], 'mean_nothf':np.mean(not_hf)[0], 'hf1_tvalue_hf2': tvalue_sg[0],'hf1_pvalue_hf2': pvalue_sg[0], 'hf1_tvalue_nothf': pvalue_all[0], 'hf1_pvalue_nothf':pvalue_all[0], 'hf1_tvalue_oneSamp':tvalue_one[0],'hf1_pvalue_oneSamp':pvalue_one[0]}, ignore_index = True)
     X['Cluster_feature'] = 0
     
st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H%M')
results.to_csv('pima-sdtock-results-' + str(st) + '.csv', index=False)   
plt.legend(loc="lower right")
plt.title("ROC Curves for Subgroup Discovery to Ckmeans (Pima Indians)")
plt.tight_layout()
plt.show()
...
# Ckmeans to SD 
#######################################################################################################
os.chdir('D:\\TTU\\NEL_Research\\SD Work\\PimaIndians\\CkmeansToSD')
...
classifier = learner(Orange.data.Table('pima-indians-cktosd-orange-format.csv'))
...
for j in range(0,5):
    ...
    c = sg[sg.Cluster2 == 1].index
    X.loc[c, 'Cluster_feature'] = 1

    # Get dropped features for those in the sg and those not in the sg.
    hf1 = dropped_feature.loc[c]
    hf2 = dropped_feature.loc[sg[sg.Cluster2==2].index]
    not_hf = dropped_feature.loc[~dropped_feature.index.isin(hf1.index)]    
    ...
    results = results.append({'SG':'SG' + str(j),'Rule': str(classifier.rule_list[j]), 'SG_size': sg.shape[0], 'count-cluster-1': len(sg[sg.Cluster2 == 1]), 'count-cluster-2': len(sg[sg.Cluster2 == 2]), 'Accuracy':accuracy, 'AUROC':roc_auc_score(y, current_probabilities), 'len_hf1':len(hf1), 'len_hf2':len(hf2), 'len_nothf': len(not_hf), 'mean_hf1':np.mean(hf1)[0], 'mean_hf2':np.mean(hf2)[0], 'mean_nothf':np.mean(not_hf)[0], 'hf1_tvalue_hf2': tvalue_sg[0],'hf1_pvalue_hf2': pvalue_sg[0], 'hf1_tvalue_nothf': tvalue_all[0], 'hf1_pvalue_nothf':pvalue_all[0], 'hf1_tvalue_oneSamp':tvalue_one[0],'hf1_pvalue_oneSamp':pvalue_one[0]}, ignore_index = True)    
    X['Cluster_feature'] = 0
st = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H%M')
results.to_csv('pima-cktosd-results-' + str(st) + '.csv', index=False)
plt.legend(loc="lower right")
plt.title("ROC Curves for Ckmeans to Subgroup Discovery (Pima Indians)")
plt.tight_layout()
plt.show()